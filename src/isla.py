...
import random
import os #Nos permitirá limpiar la consola
...
def pedir_movimiento(mapa: list) -> str:
    ...
    while not entrada_correcta:
        if entrada.lower() in MOVIMIENTOS:   #Usamos .lower para asegurarnos que la entrada está en minúscula
...
def imprimir_mapa(mapa: list):
    ...
    """
    print("    1    2    3    4    5")  #Nos muestra los números del tablero
    for fila in mapa:
...
def jugar():
    ...
    while movimiento != SALIR or resultado_movimiento != TESORO_ENCONTRADO:
        ...
        # Obtener la nueva posición del jugador y procesar el movimiento, finalizará el programa si encuentra el tesoro o el usuario decide salir. 
        if movimiento == "q" or resultado_movimiento == TESORO_ENCONTRADO:  
            movimiento = "q"
            resultado_movimiento = TESORO_ENCONTRADO
            print("Saliendo...")
        # Continuará pidiendo movimientos hasta encontrar el tesoro
        else:
